...
DAEMON_ARGS="/var/sites/borrasca/node/borrasca-next.app.js"               # node arguments ex. path to your app.js/server.js
                                            # NB: don't use ~/ in path
 
DESC="APPNAME node.js http server"          # whatever fancy description you like
 
NODEUSER=antonio:www-data                    # USER who OWNS the daemon process (no matter whoever runs the init script)
                                            # user:group (if no group is specified, the primary GID for that user is used)
 
LOCAL_VAR_RUN=/home/antonio/tmp            # in case the init script is run by non-root user, you need to
...
NAME=APPNAME_nodejs                         # name of the service
DAEMON=/usr/bin                        # this SHOULD POINT TO where your node executable is
